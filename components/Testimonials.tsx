      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="mb-16">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-[10px] font-black text-green-700 uppercase tracking-[0.4em]">Network_Authority_Feedback</span>
          </div>
          
          <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-slate-900 leading-[0.9] mb-8 uppercase">
            ELITE <br />
            <span className="text-green-600 italic font-medium">ALLIANCES.</span>
          </h2>
          
          <div className="max-w-md space-y-4 border-l-4 border-green-600 pl-6">
            ...
            <div className="flex gap-3">
               <div className="px-2 py-0.5 bg-slate-50 border border-slate-100 rounded text-[8px] font-black text-green-700 uppercase tracking-widest">
                 200+ Nodes Secured
               </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              className="group relative h-full bg-white border border-slate-200 rounded-[2rem] p-8 overflow-hidden transition-all duration-500 hover:border-green-600 shadow-premium"
            >
              {/* INTERNAL CARD DOTS */}
              <div className="absolute inset-0 btn-dot-pattern opacity-0 group-hover:opacity-10 transition-opacity z-10" />
              
              {/* Status Badge */}
              <div className="absolute top-5 right-5 px-1.5 py-0.5 bg-slate-50 border border-slate-100 rounded text-[6px] font-black text-green-700 uppercase tracking-[0.2em] z-20">
                {testimonial.status}
              </div>
              ...
              <div className="flex justify-between items-start mb-8 relative z-10">
                <div className="flex gap-1">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <Star key={i} className="w-3 h-3 fill-green-600 text-green-600 group-hover:scale-110 transition-transform duration-500" />
                  ))}
                </div>
                <Quote className="w-8 h-8 text-slate-100 group-hover:text-green-600/10 transition-colors duration-500" />
              </div>
              ...
              <div className="flex items-center gap-4 pt-8 border-t-2 border-slate-50 relative z-10">
                <div className="relative shrink-0">
                    <div className="w-12 h-12 rounded-xl overflow-hidden border-2 border-slate-100 group-hover:border-green-600/30 transition-all duration-700 shadow-sm">
                        <img
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[8px] font-black text-green-700 uppercase tracking-widest mt-0.5">
                    {testimonial.company}
                  </p>
                </div>
              </div>
              ...
              {/* Hover Glow Bloom */}
              <div className="absolute -bottom-12 -right-12 w-40 h-40 bg-green-600/0 group-hover:bg-green-600/5 blur-3xl rounded-full transition-all duration-700" />
            </motion.div>
          ))}
        </div>
      </div>
