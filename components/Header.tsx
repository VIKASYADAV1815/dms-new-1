    ...
    >
      <div 
        className={`relative flex items-center justify-between px-6 py-3 rounded-2xl transition-all duration-700 border ${
          isScrolled || isOpen
            ? 'bg-white/[0.02] backdrop-blur-2xl border-white/[0.1] shadow-[0_20px_50px_rgba(0,0,0,0.3)]' 
      ...
      >
        ...
        <div className="flex items-center gap-3 group cursor-pointer relative z-[110]">
          <div className="relative w-8 h-8 flex items-center justify-center">
            ...
            <div className="relative w-full h-full bg-white/[0.05] backdrop-blur-md rounded-lg flex items-center justify-center border border-white/20 overflow-hidden">
        ...
        <nav className="hidden md:flex items-center gap-2">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="px-4 py-1.5 text-[12px] font-medium text-white/50 hover:text-white transition-all rounded-full hover:bg-white/[0.05] relative group overflow-hidden"
            >
              {link.label}
            </a>
          ))}
        ...
        <button
          className="md:hidden relative z-[110] w-10 h-10 flex flex-col items-center justify-center gap-1.5 rounded-xl bg-white/[0.05] border border-white/10"
